...
describe("PXLProperty", function() {
  it("PXLProperty Testing Deployment", async function() {
    const PXLProperty = await ethers.getContractFactory("PXLProperty");
    const pxlProperty = await PXLProperty.deploy('0x9bc0b36cdedadb9ae906f53bdea6debe20b81b8e');
...
describe('VirtualRealEstate', function() {
  it("VirtualRealEstate Deployment", async function() {
    ...
    const virtualRealEstate = await VirtualRealEstate.deploy();

    await virtualRealEstate.deployed();
    pixelPropertyInstance = virtualRealEstate;

    await pxlPropertyInstance.setPixelPropertyContract(pixelPropertyInstance.address);
    await pixelPropertyInstance.setPXLPropertyContract(pxlPropertyInstance.address);
  });
